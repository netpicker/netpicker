...
**Important**: To enable LDAP authentication, you must set the `AUTH_BACKEND` environment variable to `ldap` in your `docker-compose.override.yml` file.
...
```yaml
# Set AUTH_BACKEND to ldap to enable LDAP authentication
AUTH_BACKEND: ldap
